...
import { fetchOfficeSpace } from "../actions/products";
...
const OfficeSpaces = () => {
  ...
  const { fetchData } = useFetchData(url, fetchOfficeSpace, "office");
  ...
  const offices = useSelector((state) => state.products.offices);
  ...
  let maxPropertyPrice = Math.max(
    ...offices.map((properties) => properties.price)
  );

  // get unique search parameters using the utility function below
  let propertyLocation = getUniqueParameter(offices, "location");
  ...
  sortedOffices = sortedOffices.filter((property) => {
  ...
  return (
    <section className="room__self__contain">
      ...
      <div>
        {!loading && !error && sortedOffices.length === 0 && <ErrorSearching />}
      </div>
      <article className="room__list">
        {sortedOffices.map((property) => {
          return (
            <SingleProperty
              key={property.id}
              property={property}
