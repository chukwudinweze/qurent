...
const RoomSelfContain = () => {
  ...
  let sortedRoomSelfcons = selfContains;

  // get maximum price
  let maxPropertyPrice = Math.max(
    ...selfContains.map((properties) => properties.price)
  );

  // get unique search parameters using the utility function below
  let propertyLocation = getUniqueParameter(selfContains, "location");
  ...
  return (
    <>
      <section className="room__self__contain">
        ...
        <div>
          {!loading && !error && sortedRoomSelfcons.length === 0 && (
            <ErrorSearching />
          )}
        </div>
